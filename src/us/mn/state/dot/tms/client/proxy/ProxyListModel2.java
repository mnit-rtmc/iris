 ...
 * @author Douglas Lau
 */
public class ProxyListModel2<T extends SonarObject>
	extends AbstractListModel
{
	...
	private final SwingProxyAdapter<T> listener = new SwingProxyAdapter<T>() {
		protected Comparator<T> comparator() {
			return ProxyListModel2.this.comp;
		}
	...
	public ProxyListModel2(TypeCache<T> c) {
		cache = c;
	...
	protected boolean isIncluded(T proxy) {
		return true;
	}
	...
	private int doProxyAdded(T proxy) {
		if (isIncluded(proxy)) {
			...
			for (int i = 0; i < n_size; ++i) {
				if (comp.compare(proxy, list.get(i)) < 0) {
					list.add(i, proxy);
					return i;
				}
			}
