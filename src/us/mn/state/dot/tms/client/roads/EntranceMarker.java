...
/**
 * Marker used to paint roadway entrance nodes.
 *
 * @author Douglas Lau
 */
public class EntranceMarker extends AbstractMarker {

	/** Size (in user coordinates) to render marker */
	static protected final int MARKER_SIZE = 300;

	/** Create a new roadway entrance node marker */
	public EntranceMarker() {
		this(MARKER_SIZE);
	}

	/** Create a new roadway entrance node marker */
	public EntranceMarker(float size) {
		super(4);
		float half = size / 2;
		float third = size / 3;
		float sixth = size / 6;
