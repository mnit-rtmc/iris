...
public class WMsgConfirmDeleteForm extends AbstractForm {
	...
	public WMsgConfirmDeleteForm(Session s, WMsgSelectorForm sForm, String mName) {
		super(I18N.get("wysiwyg.confirm.title"), true);
		session = s;
		selectorForm = sForm;
		messageName = mName;
	...
	protected void initialize() {
		...
		gbc.gridx = 0;
		gbc.gridy = 0;
		p.add(new JLabel(String.format(I18N.get("wysiwyg.confirm.message"), messageName)), gbc);
	...
	private final IAction delete = new IAction(
		...
		protected void doActionPerformed(ActionEvent e)
				throws Exception {
			// call the selector form's deleteSelectedMessage method then close the warning
