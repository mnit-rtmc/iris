...
import us.mn.state.dot.tms.client.notification.NotificationForm;
	...
	private final IAction openNotifGui = new IAction("notification") {
		...
		protected void doActionPerformed(ActionEvent ev) throws Exception {
			session.getDesktop().show(
					new NotificationForm(session, map, pane));
		}
	};

	public NotificationPanel(Session s, MapBean m, ScreenPane p) {
		session = s;
		map = m;
		pane = p;
		notificationBtn = new JButton(openNotifGui);
		add(notificationBtn);
		manager = session.getNotificationManager();
