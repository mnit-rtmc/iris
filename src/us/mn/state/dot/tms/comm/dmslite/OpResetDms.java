...
{
	protected static final String OPNAME="OpResetDms";
	...
	protected Phase phaseOne() {
		System.err.println("dmslite.OpResetDms.phaseOne() called.");
	...
	protected class PhaseResetDms extends Phase
	{
		...
		protected Phase poll(AddressedMessage argmess) throws IOException {
			System.err.println(
			    "OpResetDms.PhaseResetDms.poll(msg) called.");
			...
			ReqRes rr1 = new ReqRes("Address", addr, new String[0]);
			mess.add(rr1);
			...
			try {
				...
				System.err.println(
				    "OpResetDms.PhaseResetDms: Malformed XML received:"+ ex+", id="+id);
				valid=false;
			...
			// valid flag is false
				...
				System.err.println(
				    "OpResetDms: response from cmsserver received, ignored because Xml valid field is false, errmsg="+errmsg);
				m_dms.setStatus(OPNAME+": "+errmsg);

				// try again
				if (flagFailureShouldRetry(errmsg)) {
					System.err.println("OpResetDms: will retry failed operation.");
					return this;
				}
			}
