...
public class OpQueryEssStatus extends OpEss {
	...
	private final SpectroTable spectro_table;
	...
	public OpQueryEssStatus(WeatherSensorImpl ws) {
		...
		spectro_table = ess_rec.spectro_table;
	}
	...
	protected class QueryPavementRow extends Phase {
		...
		protected Phase poll(CommMessage mess) throws IOException {
			...
			try {
				...
				log("QueryPavementRow: ignored ex=" + ex);
			}
	...
	/** Phase to Vaisala-specific spectro measurements */
	protected class QuerySpectro extends Phase {

		/** Query */
		@SuppressWarnings("unchecked")
		protected Phase poll(CommMessage mess) throws IOException {
			mess.add(spectro_table.num_sensors);
			try {
				mess.queryProps();
				logQuery(spectro_table.num_sensors);
			} catch(NoSuchName ex) {
				log("QuerySpectro: ignored ex=" + ex);
			}
			return spectro_table.isDone()
					? null
					: new QuerySpectroRow();
		}
	}

	/** Phase to query all rows in Vaisala spectro table */
	protected class QuerySpectroRow extends Phase {
		private final SpectroTable.Row sr = spectro_table.addRow();
		...
		protected Phase poll(CommMessage mess) throws IOException {
			try {
				mess.add(sr.loc);
				mess.add(sr.friction.node);
				mess.add(sr.ice_layer);
				mess.add(sr.snow_layer);
				mess.add(sr.water_layer);
				mess.add(sr.air_temp.node);
				mess.add(sr.rh.node);
				mess.add(sr.dewpoint.node);
				mess.add(sr.surface_temp.node);
				mess.add(sr.surface_status);
				mess.add(sr.hardware_status);
				mess.queryProps();
				logQuery(sr.friction.node);
				logQuery(sr.ice_layer);
				logQuery(sr.snow_layer);
				logQuery(sr.water_layer);
				logQuery(sr.air_temp.node);
				logQuery(sr.rh.node);
				logQuery(sr.dewpoint.node);
				logQuery(sr.surface_temp.node);
				logQuery(sr.surface_status);
				logQuery(sr.hardware_status);
			} catch(NoSuchName ex) {
				log("QuerySpectroTable: ignored ex=" + ex);
			}
			return spectro_table.isDone()
					? null
					: new QuerySpectroRow();
