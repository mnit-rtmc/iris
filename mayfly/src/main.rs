...
use common::Error;
use query::{
    DateQuery, DetectorQuery, DistrictQuery, OccupancyData, SpeedData,
    TrafficQuery, VehicleData, YearQuery,
};
...
async fn main() -> tide::Result<()> {
    ...
    root.at("vehicles").get(|req| handle_vehicles(req));
...
async fn handle_vehicles(req: Request<()>) -> tide::Result {
    handle_query!(req, TrafficQuery<VehicleData>)
}
